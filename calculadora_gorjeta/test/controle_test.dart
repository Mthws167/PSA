...
void main() {
  test("Gorjeta não é obrigatória", () {
   ...
   test("A gorjeta, quando oferecida, não deverá ser negativa", () {
    ...
    expect(Controle().gorjetaNegativa(cliente.gorjeta?.valor), 0.0);
  });
  ...
  test("A gorjeta deve ter valor mínimo de 5% do valor da conta e o valor máximo de 30% do valor da conta, quando oferecida.", () {
    ...
    expect(Controle().gorjetaMaxima(cliente.gorjeta?.valor), 30.0);
    expect(Controle().gorjetaMinima(cliente1.gorjeta?.valor), 5.0);
    expect(Controle().gorjetaMaxima(cliente2.gorjeta?.valor), 30.0);
  });
